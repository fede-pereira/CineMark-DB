...
def generate_peliculas_data(num_rows):
    for _ in range(num_rows):
        ...
        genero_pelicula = fake.random_element(elements=("Action", "Comedy", "Drama", "Thriller"))
        yield (nombre_pelicula, director, duracion_en_segundos, estudio, presupuesto, tiempo_de_realizacion, ano_estreno, genero_pelicula)
...
def generate_compras_data(num_rows, funciones_data, clientes_data):
    for _ in range(num_rows):
        nombre_pelicula, director, id_sala, id_cine, ts = fake.random_element(funciones_data)
...
cur.executemany("INSERT INTO salas (id_sala, id_cine, asientos) VALUES (%s, %s, %s)", salas_data)

# Insert data into peliculas table
cur.executemany("INSERT INTO peliculas (nombre_pelicula, director, duracion_en_segundos, estudio, presupuesto, tiempo_de_realizacion, ano_estreno, genero_pelicula) VALUES (%s, %s, %s, %s, %s, %s, %s, %s)", peliculas_data)

# Insert data into actores table
cur.executemany("INSERT INTO actores (nombre, apellido, edad, genero) VALUES (%s, %s, %s, %s)", actores_data)

# Insert data into clientes table
cur.executemany("INSERT INTO clientes (cuit, nombre, apellido, edad, nacionalidad, genero) VALUES (%s, %s, %s, %s, %s, %s)", clientes_data)
...
cur.executemany("INSERT INTO funciones (nombre_pelicula, director, id_sala, id_cine, ts) VALUES (%s, %s, %s, %s, %s)", funciones_data)

# Insert data into compras table
compras_data = list(generate_compras_data(num_rows, funciones_data, clientes_data))
cur.executemany("INSERT INTO compras (nombre_pelicula, director, id_sala, id_cine, ts, cuit, cantidad) VALUES (%s, %s, %s, %s, %s, %s, %s)", compras_data)
...
cur.executemany("INSERT INTO actua (nombre_pelicula, director, id_actor, salario) VALUES (%s, %s, %s, %s)", actua_data)
